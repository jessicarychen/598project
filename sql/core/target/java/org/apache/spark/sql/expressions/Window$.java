...
public  class Window$ {
   ...
   * from <code>start</code> (inclusive) to <code>end</code> (inclusive).
   * <p>
   * Both <code>start</code> and <code>end</code> are relative to the current row. For example, "lit(0)" means
   * "current row", while "lit(-1)" means one off before the current row, and "lit(5)" means the
   * five off after the current row.
   * <p>
   * Users should use <code>unboundedPreceding()</code>, <code>unboundedFollowing()</code>, and <code>currentRow()</code> from
   * {@link org.apache.spark.sql.functions} to specify special boundary values, literals are not
   * transformed to {@link org.apache.spark.sql.catalyst.expressions.SpecialFrameBoundary}s.
   * <p>
   * A range-based boundary is based on the actual value of the ORDER BY
   * expression(s). An offset is used to alter the value of the ORDER BY expression, for
   * instance if the current order by expression has a value of 10 and the lower bound offset
   ...
   *              {@link org.apache.spark.sql.catalyst.expressions.UnboundedPreceding}.
   * @param end boundary end, inclusive. The frame is unbounded if the expression is
   *            {@link org.apache.spark.sql.catalyst.expressions.UnboundedFollowing}.
   * @since 2.3.0
   * @return (undocumented)
   */
